<template>
  ...
  >
    <div class="va-tab__content">
      <slot>
        <va-icon v-if="icon" class="va-tab__icon" size="small" :name="icon" />
        <span class="va-tab__label" v-text="label" />
      </slot>
    </div>
  </component>
</template>
...
const { parentDisabled, selectTab, moveToTab, registerTab, unregisterTab } = inject<TabsView>(
  TabsViewKey,
  {
